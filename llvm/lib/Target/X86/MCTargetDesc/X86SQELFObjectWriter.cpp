//===-- X86SQELFObjectWriter.cpp - X86 SQELF Writer ---------------------------===//
//
...
class X86SQELFObjectWriter : public MCSQELFObjectTargetWriter {
public:
  X86SQELFObjectWriter(bool IsELF64, uint8_t OSABI, uint16_t EMachine);
  ~X86SQELFObjectWriter() override = default;
...
X86SQELFObjectWriter::X86SQELFObjectWriter(bool IsELF64, uint8_t OSABI,
                                       uint16_t EMachine)
    : MCSQELFObjectTargetWriter(IsELF64, OSABI, EMachine,
...
unsigned X86SQELFObjectWriter::getRelocType(MCContext &Ctx, const MCValue &Target,
                                          const MCFixup &Fixup,
                                          bool IsPCRel) const {
...
llvm::createX86SQELFObjectWriter(bool IsELF64, uint8_t OSABI, uint16_t EMachine) {
  std::cout<<"Create x86sqelf object writer"<<std::endl;
  return std::make_unique<X86SQELFObjectWriter>(IsELF64, OSABI, EMachine);
}
